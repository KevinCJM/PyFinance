...
def read_data(nv_parquet, trade_day_parquet):
    """
    读取并预处理ETF净值数据和交易日历数据，计算ETF日收益率并返回透视后的结果。

    :param nv_parquet: str, ETF净值数据的Parquet文件路径
    :param trade_day_parquet: str, 交易日历数据的Parquet文件路径
    :return: pandas.DataFrame, 以日期为索引、ETF代码为列名、收益率为值的透视表
    """
    # 读取数据以及预处理
