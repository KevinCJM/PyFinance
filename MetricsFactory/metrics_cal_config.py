...
rolling_days = {
    '布林带相关指标': [
        [5, 10, 15, 20, 25, 30, 60, 99],  # 参数: 滚动天数
        {
            'PriceSigma':
                ['过去N天收盘价的滚动标准差',  # 指标名称
    ...
    'KDJ相关指标': [
        [5, 10, 15, 20, 25, 30, 60, 99, 0],  # 0表示存在计算完 滚动指标 后才能计算的指标
        {
            'L':
                ['过去N天的最低价 (KDJ相关指标)',
                 'L = Min(N天最低价)',
            ...
            'KDJ-K':
                ['KDJ 指标的 K 值',
                 'K = RSV 的指数移动平均值',
                 [(5, 3), (10, 3), (15, 3), (20, 3), (25, 3), (30, 3), (60, 3), (99, 3)]  # 额外参数列表: (滚动天数,平滑天数)
                 ],
            'KDJ-D':
                ['KDJ 指标的 D 值',
                 'D = K 的指数移动平均值',
                 ['Same as KDJ-K']  # 额外参数列表: (滚动天数,平滑天数)
                 ],
            'KDJ-J':
                ['KDJ 指标的 J 值',
                 'J = 3 * K - 2 * D',
        ...
        [5, 10, 15, 20, 25, 30, 60, 99, 0],  # 0表示存在计算完 滚动指标 后才能计算的指标
        {
            ...
            'DIF':  # 不属于滚动指标, 属于EMA的衍生指标 (滚动衍生指标)
                ['MACD 指标的 DIF 值 (快线-趋势变化)',
                 'DIF = EMA(短期) - EMA(长期)',
                 [(12, 26, 9), (6, 13, 5), (10, 23, 8)]  # 额外参数列表: (EMA短周期, EMA长周期, DEA平滑周期)
                 ],
            'DEA':  # 不属于滚动指标, 属于EMA的衍生指标 (滚动衍生指标)
                ['MACD 指标的 DEA 值 (慢线-趋势平滑)',
                 'DEA = DIF 的指数移动平均值',
                 ['Same as DIF']  # 额外参数列表: (EMA短周期, EMA长周期, DEA平滑周期)
                 ],
