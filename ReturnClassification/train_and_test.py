...
def split_train_test_data(fund_code, future_days, df_price, metrics_df,
    ...
    print(f"训练集时间区间: {train_start} ~ {train_end}")
    print(f"测试集时间区间: {test_start} ~ {test_end}")
    ...
    print(f"测试集数据量: {len(test_df)}, 共 {len(test_df.columns)} 列")
...
def train_and_test_random_forest(x_train, x_test, y_train, y_test, feature_cols,
...
def predict_main_random_forest(the_fund_code='159919.SZ', n_days=20,
                               folder_path='../Data', metrics_folder='../Data/Metrics'
                               ):
    """
    主函数，用于执行基金数据预处理、模型训练等任务。

    :param the_fund_code: 基金代码，默认为'159919.SZ'
    :param n_days: 用于计算未来对数收益率的天数，默认为20天
    :param folder_path: 数据文件夹路径，默认为'../Data'
    :param metrics_folder: 指标数据文件夹路径，默认为'../Data/Metrics'
    :return: 训练完成的模型
    """
    ''' 价格数据预处理 '''
    # 获取收盘价数据
...
if __name__ == '__main__':
    predict_main_random_forest(the_fund_code='159919.SZ', n_days=25,
                               folder_path='../Data', metrics_folder='../Data/Metrics',
                               )
