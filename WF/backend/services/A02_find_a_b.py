...
def find_a_points(
    ...
    cond1_ok_series = (maL_t1.notna() & maL_t1_L.notna()) & (maL_t1 < maL_t1_L)
    ...
    for k in short_windows:
        ...
        d[above_col] = d[ma_s] > d[ma_long]
        prev_above = g[ma_s].shift(1) > g[ma_long].shift(1)
    ...
    if (confirm_lookback_days > 0) and (confirm_ma_window is not None) and enable_cond3:
        ...
        confirm_cnt = g["_cross_price_up"].transform(
            lambda s: s.shift(1).rolling(confirm_lookback_days, min_periods=confirm_lookback_days).sum()
        )
        d["confirm_cross_cnt"] = confirm_cnt
        cond3_ok_series = (confirm_cnt >= 1)
        cond3_expect_str = f"A点前{confirm_lookback_days}日内，{price_col}由下上穿MA{confirm_ma_window}≥1次（区间[t-N,t-1]）"
        # 实际：给出次数
    ...
    # ---------- VR1 ----------
    prev10max = g[volume_col].transform(lambda s: s.shift(1).rolling(10, min_periods=1).max())
...
if __name__ == '__main__':
    ...
    # 读取文件
    print("读取的 DataFrame：")
    stock_df = pd.read_parquet(f'{code}.parquet')
    print(stock_df.tail())
