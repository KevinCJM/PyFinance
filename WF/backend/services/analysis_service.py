...
def find_b_points(
    ...
    if c1_max_days_from_a is not None and c1_max_days_from_a >= 0:
        cond1_time_ok = (d["seg_id"] > 0) & (d["days_since_A"] >= c1_min_days_from_a) & (d["days_since_A"] <= c1_max_days_from_a)
    else:
    ...
    if c2_above_maN_consecutive:
        ...
        if c2_above_maN_ratio is not None:
            ...
            maN_actual_text = ("MA{}在长期线上比例=".format(c2_above_maN_window) + ratio_pct +
                               ", 上方天数(A..t)=" + d["maN_above_since_A_incl_today"].fillna(0).astype(int).astype(str) +
    ...
    cond3_close_le_prev_ok = (d[close_col] <= prev_close) if c3_require_close_le_prev else pd.Series(True, index=d.index)
    ...
    d["cond4_expect"] = pd.Series(
        (f"VR1≤{c4_vr1_max}（近{c4_recent_max_win}日最大量为参照）" if c4_vr1_max is not None else "（未设置VR1阈值）"), index=d.index
    )
    ...
    c5_vol_cmp_enabled = bool(_c5_dry.get("vol_cmp_enabled", (_c5_dry.get("require_vol_le_vma10", require_vol_le_vma10) if not has_new_flags else False)))
    ...
    c5_vma_short = int(_c5_dry.get("vma_short_window", _c5_dry.get("short_days", 5))) if _c5_dry.get("vma_short_window", None) not in (None, "") else 5
    c5_vma_long = int(_c5_dry.get("vma_long_window", _c5_dry.get("long_days", 10))) if _c5_dry.get("vma_long_window", None) not in (None, "") else 10
    # 均量比较的“长天数”可单独配置；未提供时回退为比例上限的长天数
    c5_vol_cmp_long = int(_c5_dry.get("vol_compare_long_window", c5_vma_long)) if _c5_dry.get("vol_compare_long_window", None) not in (None, "") else c5_vma_long
    ...
    if need_vma:
        vma_short = g_code[volume_col].transform(lambda s: s.rolling(int(max(1, c5_vma_short)), min_periods=int(max(1, c5_vma_short))).mean())
        vma_long = g_code[volume_col].transform(lambda s: s.rolling(int(max(1, c5_vma_long)), min_periods=int(max(1, c5_vma_long))).mean())
        vma_long_cmp = g_code[volume_col].transform(lambda s: s.rolling(int(max(1, c5_vol_cmp_long)), min_periods=int(max(1, c5_vol_cmp_long))).mean())
    else:
    ...
    if has_new_flags:
        ...
        cond5_vol_ok = (d[volume_col] <= vma_long_cmp) if bool(_c5_dry.get("require_vol_le_vma10", require_vol_le_vma10)) else pd.Series(True, index=d.index)
        ...
        d["cond5_expect"] = (
                ...
                + ("" if bool(_c5_dry.get("require_vol_le_vma10", require_vol_le_vma10)) is False else f"；量≤VMA{c5_vol_cmp_long}")
        ...
        vol_part = ("" if bool(_c5_dry.get("require_vol_le_vma10", require_vol_le_vma10)) is False else ("，vol≤vma_long_cmp=" + cond5_vol_ok.astype(bool).astype(str)))
