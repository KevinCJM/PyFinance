...
CONFIG = {
    ...
    "environment": {
        ...
        "transaction_cost_pct": 0.01,  # 手续费依然保留
        "trade_penalty": 0.2,  # 固定的交易行为惩罚
...
class StockTradingEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.history = []
        self.entry_price = 0
        self.entry_step = 0  # 新增，记录持仓的步数
        return self._get_observation()

    def _get_observation(self):
        market_obs = self.features_array[self.current_step]
        position_status = 1.0 if self.shares > 0 else 0.0
        holding_period = self.current_step - self.entry_step if self.shares > 0 else 0.0

        return np.concatenate([
            market_obs,
            [position_status, holding_period]
        ]).astype(np.float32)
    ...
    def step(self, action):
        ...
        # 1. 计算单步收益
        portfolio_return = np.log(self.portfolio_value / prev_portfolio_value)
        benchmark_return = np.log(
            self.price_series.iloc[self.current_step if not done else -1] /
            self.price_series.iloc[self.current_step - 1]
        )

        # 下行波动率计算
        window = 21
        returns = self.price_series.pct_change().iloc[max(0, self.current_step - window):self.current_step]
        downside_returns = returns[returns < 0]
        downside_vol = downside_returns.std() if len(downside_returns) > 1 else returns.std()

        if downside_vol > 1e-8:
            reward = (portfolio_return - benchmark_return) / downside_vol
        else:
            reward = portfolio_return - benchmark_return

        # 2. 明确的交易成本惩罚
        if action in [1, 2]:
            trade_cost = self.transaction_cost_pct * prev_portfolio_value
            reward -= trade_cost
            reward -= self.trade_penalty  # 固定惩罚项

        # 3. 鼓励长期趋势持仓
        if self.shares > 0:
            holding_return = (current_price / self.entry_price) - 1
            # 仅在盈利时给予持续奖励
            if holding_return > 0:
                reward += 0.2 * holding_return
            else:
                reward += 0.2 * holding_return  # 惩罚亏损持仓
    ...
    def _take_action(self, action):
        ...
        if action == 1 and self.cash > 0:
            ...
            if buy_price > 0:
                ...
                self.entry_step = self.current_step  # 新增：记录买入时的步数
